  ...
  <body>
	...
	<script>
		...
		$(document).ready(function(){
			$.ajax({
				type: "POST",
				url: '/xxzx/officeSupplies/list.do',
			    data: {},
				dataType:'json',
				cache: false,
				success: function(data){
					...
					//mui.alert(data.code, 'Hello MUI'); 
						var html_list="";
						if(data.code == 0){
							//alert(data.data[0][0], 'Hello MUI');
						  for(var i=0;i<data.data.length;i++){
							html_list= html_list+'<div class="mui-input-row"><label><h6>'+data.data[i][1]+
								      //'<div id="M_Toggle" class="mui-switch mui-active">'+
								      '</h6></label><div class="mui-numbox">'+
									  '<button class="mui-btn mui-btn-numbox-minus" type="button" onclick="zj(this);" id="b_'+data.data[i][0]+'">-</button>'+
									  '<input class="mui-input-numbox" type="number" value="0" id="'+data.data[i][0]+'" />'+
									  '<button class="mui-btn mui-btn-numbox-plus" onclick="add(this);"  id="a_'+data.data[i][0]+'" type="button">+</button>'+
									  //'</div>'+
								      '</div></div>';
					       }
